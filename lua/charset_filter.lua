...
local function should_yield(text, tag, charsetdb)
  local should_yield = true
  for i in utf8.codes(text) do
    local code = utf8.codepoint(text, i)
    if is_cjk(code) then -- 只检查 CJK 字符，其它字符不参与过滤
      local charset = charsetdb:lookup(utf8.char(code))
...
local function filter(input, env)
  extended_char_on = env.engine.context:get_option("extended_char")
  for cand in input:iter() do
    if extended_char_on then
      if should_yield(cand.text, '[c]', env.charsetdb) then  -- custom charset range
        yield(cand)
      end
    else
      yield(cand)
...
local function init(env)
  -- 当此组件被载入时，打开反查库，并存入 `charsetdb` 中
  env.charsetdb = ReverseDb("build/charset_filter.reverse.bin")
end

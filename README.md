...
## 虎码
由秃版单字版精简而成，词库只保留了 9767 个常用字，支持字根拆分显示（需要 TumanPUA 字体）。
- 虎码链接：[官方网站](https://www.tiger-code.com/)
- 虎码秃版：[压缩包](https://ys-e.ysepan.com/wap/huma/RkH8DaBQ5RhSfEE/9c6fD6jq4hDhGQa8Bs2IEfD2fV1iE3/%E8%99%8E%E7%A0%81%E7%A7%83%E7%89%88%20%E5%B0%8F%E7%8B%BC%E6%AF%AB%EF%BC%88Win%EF%BC%892026.03.01.7z)

1. 使用 ` 键拼音反查
1. 使用 * 键通配任意字符
1. 使用 ~ 键以形查音（只支持单字）
1. 使用 = 键触发计算器
1. 挂载了英文词库，直接输入英文单词即可，英文权重低于中文
...
1. 使用 z 键拼音反查
1. 使用 ~ 键以形查音（只支持单字）
1. 使用 ' 键分词，分词后的产生的短语上屏后会自动造词
1. 使用 = 键触发计算器
1. 在 opencc/personal.txt 中添加个人隐私信息，例如手机号、地址等
1. 在 wubi.extended.dict.yaml 中添加个人常用词，例如人名，常打的特殊短语等
1. 特定时间相关词可以用 `,` 上屏时间串，如 `20240601`，用`-`可上屏`2024-06-01`，`/`可上屏`2024/06/01`
1. 挂载了英文词库，直接输入英文单词即可，英文权重低于中文
